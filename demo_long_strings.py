...
config_path = r"C:\Users\Administrator\Documents\MyProjects\PythonApplication\C" \
              r"onfiguration\Settings\Production\database_config.ini"
...
# Long API URL
api_endpoint = "https://api.example.com/v2/users/profile/settings/notifications" \
               "/preferences/email?include_metadata=true&detailed=true&format=j" \
               "son"
...
log_warning = "Database connection pool is running low on available " \
              "connections - consider increasing the pool size in configuration"
...
def calculate_stats(data):
    result = f"Analysis complete: Mean={sum(data)/len(data)}, Min={min(data)}, " \
             f"Max={max(data)}, Total items processed={len(data)} from dataset"
    return result
...
# This is a very long comment that exceeds the maximum line length and will not
# be split by the string fixer because it's not a string literal

# 2. IMPORT STATEMENTS - Long imports won't be split
# from some_very_long_module_name_that_exceeds_the_maximum_line_length_and_shou
# ld_not_be_split import some_function, another_function, yet_another_function

# 3. FUNCTION DEFINITIONS - Long function signatures won't be split
# def some_function_with_many_parameters(parameter_one, parameter_two,
# parameter_three, parameter_four, parameter_five, parameter_six):
#     pass

# 4. CLASS DEFINITIONS - Long class inheritance won't be split
# class SomeClassWithVeryLongInheritance(BaseClassOne, BaseClassTwo,
# BaseClassThree, BaseClassFour, BaseClassFive):
#     pass
...
# if some_condition and another_condition and yet_another_condition and
# one_more_condition:
#     do_something()
...
# result = calculate_something() + process_data() + validate_input() +
# save_to_database()

# 13. ASSERT STATEMENTS - Won't be split
# assert condition_one and condition_two and condition_three and
# condition_four, "All conditions must be true"
...
# @some_decorator_with_many_parameters(param1="value1", param2="value2",
# param3="value3")
...
# result = "success" if condition_one and condition_two and condition_three
# else "failure"
...
# complex_fstring = f"Result:
# {get_data()['users'][0]['profile']['settings']['preferences']['theme']}"
...
# config_line = "database_url=mysql://user:password@host:port/database?charset=
# utf8&autocommit=true"

# 25. BINARY DATA STRINGS - May not be appropriate to split
# binary_data = "xff\\x00\\x01\\x02\\x03\\x04\\x05\\x06\\x07\\x08\\x09\\x0a\\x0
# b\\x0c\\x0d\\x0e\\x0f" * 10
